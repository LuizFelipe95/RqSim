    ...
    public static double CalculateAverageRicciCurvature(RQGraph graph, double sampleFraction = 1.0)
    {
        ...
        for (int i = 0; i < graph.N; i++)
        {
            foreach (int j in graph.Neighbors(i))
            {
                ...
                // Skip edges based on sample fraction
                if (rng != null && rng.NextDouble() > sampleFraction)
                {
                    continue;
                }

                // Use fast Jaccard approximation for curvature
                double curvature = RQSimulation.GPUOptimized.OllivierRicciCurvature.ComputeOllivierRicciJaccard(graph, i, j);
