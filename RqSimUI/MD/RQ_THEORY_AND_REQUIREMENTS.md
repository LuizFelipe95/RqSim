...
This document presents the **Relational-Quantum (RQ) hypothesis** in its pure theoretical form.

**Dec 2025 note:** the repository now contains multiple execution backends (CPU / Dense GPU / CSR GPU). The core theory below stays unchanged; an appendix at the end documents the **current implementation alignment**.
...
**Note on Precision**: The GPU Heat Kernel implementation (`GpuHeatKernelEngine`) was deprecated due to float32 precision limitations (~26% error vs double64). Scientific validation of spectral dimension uses the CPU double-precision implementation.
