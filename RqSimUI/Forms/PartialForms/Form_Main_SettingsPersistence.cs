...
public partial class Form_Main
{
    ...
    private void BtnSavePreset_Click(object? sender, EventArgs e)
    {
        ...
        {
            _settingsManager.CaptureCurrentState();
            var presetName = System.IO.Path.GetFileNameWithoutExtension(dialog.FileName);
            PhysicsSettingsSerializer.ExportPreset(_settingsManager.CurrentConfig, presetName);

            if (_lblSettingsStatus != null)
            {
                _lblSettingsStatus.Text = $"? Saved: {presetName}";
                _lblSettingsStatus.ForeColor = Color.DarkGreen;
            }

            AppendSysConsole($"[Settings] Preset saved: {presetName}\n");
        }
        catch (Exception ex)
        {
    ...
    private void BtnLoadFromConsole_Click(object? sender, EventArgs e)
    {
        using var dialog = new OpenFileDialog
        {
            ...
            Title = "Load Console Configuration",
        ...
        try
        {
            var config = LoadConsoleConfigAndConvert(dialog.FileName);
            if (config is not null)
            {
                ...
                if (_lblSettingsStatus != null)
                {
                    _lblSettingsStatus.Text = $"? Loaded: {Path.GetFileName(dialog.FileName)}";
                    _lblSettingsStatus.ForeColor = Color.DarkGreen;
                }

                AppendSysConsole($"[Settings] Loaded console config: {dialog.FileName}\n");
            }
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Failed to load console config: {ex.Message}", "Error",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            AppendSysConsole($"[Settings] Failed to load console config: {ex.Message}\n");
        }
    }
