...
public partial class Form_Main
{
    ...
    private void Timer3D_Tick(object? sender, EventArgs e)
    {
        ...
        // GDI+ visualization only for non-CSR modes
        if (_isExternalSimulation)
        {
            UpdateSnapshot(null, _externalNodesBuffer);
        }
    ...
    private void UpdateSnapshot(RQGraph? graph, RenderNode[]? externalNodes = null)
    {
        try
        {
            ...
            else if (externalNodes != null && externalNodes.Length > 0)
            {
                ...
                {
                    NodeCount = count,
                    SpectralDim = 0, // Not available in RenderNode
                    X = new float[count],
                    Y = new float[count],
                    Z = new float[count],
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    // Map color to state if possible, or default
                    // For now, just default state
                    snapshot.States[i] = NodeState.Excited;
                }

                // No edges or clusters for external sim yet
            }
    ...
    /// 
    /// Energy function: E = Σ(w_ij * |r_i - r_j|²) + k * Σ(1/|r_i - r_j|)
