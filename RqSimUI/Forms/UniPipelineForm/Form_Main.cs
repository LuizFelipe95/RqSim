...
using RqSim.PluginManager.UI.Configuration;
...
partial class Form_Main
{
    ...
    // Topology debug controls
    private TextBox? _txtTopologyLog;
    private Button? _btnForceCpuFallback;
    ...
    private void InitializeUniPipelineTab()
    {
        ...
        // Initialize topology debug controls (log + force fallback)
        InitializeTopologyDebugControls();
        
        // Initialize Dynamic Physics Parameters UI (Phase 4)
        InitializeDynamicPhysicsControls();
    }

    private void InitializeTopologyDebugControls()
    {
        try
        {
            // Create a small panel docked at bottom of the form to host controls
            var panel = new Panel
            {
            ...
            _btnForceCpuFallback = new Button
            {
                Text = "Force CPU Fallback",
                Dock = DockStyle.Right,
                Width = 160,
                Margin = new Padding(6)
            };
            _btnForceCpuFallback.Click += BtnForceCpuFallback_Click;

            // Add controls to panel and then to Form
            panel.Controls.Add(_txtTopologyLog);
            panel.Controls.Add(_btnForceCpuFallback);

            // Add panel to this form/tab - place at end of controls
            this.Controls.Add(panel);

            splitpanels_Add.Panel1.Controls.Add(panel);
        }
        catch
        {
            // Non-fatal; UI may not be fully initialized
        }
    }

    private void BtnForceCpuFallback_Click(object? sender, EventArgs e)
    {
        try
        {
            // Force topology mode to CSR static (CPU fallback)
            _simApi?.SetTopologyMode(RQSimulation.GPUCompressedSparseRow.GpuCayleyEvolutionEngineCsr.TopologyMode.CsrStatic);
            AppendTopologyLog("[UI] Forced CPU fallback: topology mode set to CsrStatic\r\n");
        }
        catch (Exception ex)
        {
            AppendTopologyLog($"[UI] Failed to force CPU fallback: {ex.Message}\r\n");
        }
    }

    public void AppendTopologyLog(string message)
    {
        if (_txtTopologyLog is null) return;
        if (_txtTopologyLog.InvokeRequired)
        {
            _txtTopologyLog.BeginInvoke(new Action(() => AppendTopologyLog(message)));
            return;
        }
        _txtTopologyLog.AppendText(message);
    }
