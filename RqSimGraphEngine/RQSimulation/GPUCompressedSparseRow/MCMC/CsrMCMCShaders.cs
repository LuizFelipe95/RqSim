...
/// <summary>
/// CSR Metropolis acceptance kernel.
/// 
...
public readonly partial struct CsrMetropolisAcceptKernelDouble : IComputeShader
{
    ...
    public void Execute()
    {
        ...
        {
            // ComputeSharp Exp only supports float, but we need double precision logic elsewhere.
            // We cast to float only for the probability calculation which is stochastic anyway.
            double acceptProb = (double)Hlsl.Exp((float)(-beta * dS));
            accept = randomNumbers[p] < acceptProb;
        }
...
/// <summary>
/// Enhanced Metropolis acceptance with explicit Hamiltonian constraint.
...
public readonly partial struct CsrMetropolisWithHamiltonianKernel : IComputeShader
{
    ...
    public void Execute()
    {
        ...
        // Standard Metropolis acceptance for action
        double dS = deltaActions[p];
...
/// Computes acceptance probability and applies move if accepted.
/// </summary>
[ThreadGroupSize(DefaultThreadGroupSizes.X)]
[GeneratedComputeShaderDescriptor]
[RequiresDoublePrecisionSupport]
public readonly partial struct MetropolisKernel : IComputeShader
{
    ...
    public void Execute()
    {
        ...
        // If H deviates from zero more than numerical tolerance — it is not a physical state.
        // Here we approximate H check via action deviation or explicit H calculation if available.
        // For now, we use a placeholder check or rely on dS being the effective Hamiltonian difference.
        
        // If dS is too large (energy violation), we might reject immediately regardless of temperature
        // This is a "Strict Hamiltonian Constraint" approximation
        if (Hlsl.Abs(dS) > hamiltonianTolerance * 1000.0) // Example threshold
        {
            // REJECT MOVE IMMEDIATELY
            return;
        }

        // Metropolis acceptance probability: min(1, exp(-beta * dS))
        double acceptanceProb = 1.0;
        if (dS > 0)
        {
            // ComputeSharp Exp only supports float
            acceptanceProb = (double)Hlsl.Exp((float)(-beta * dS));
        }
