...
/// <summary>
/// Metropolis acceptance kernel for batched proposals.
...
public readonly partial struct MetropolisAcceptKernelDouble : IComputeShader
{
    ...
    public void Execute()
    {
        ...
        // Metropolis criterion: P_accept = min(1, exp(-?*?S))
        bool accept;
        if (dS <= 0.0)
        {
            // Action decreases or stays same: always accept
            accept = true;
        }
        else
        {
            // Action increases: accept with probability exp(-?*?S)
            double acceptProb = Hlsl.Exp((float)(-beta * dS));
