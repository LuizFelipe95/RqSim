...
public sealed class GpuMCMCEngine : IDisposable
{
    ...
    /// <summary>
    /// Perform one Metropolis-Hastings step with gauge invariance check.
    /// </summary>
    ...
    /// <summary>
    /// Batch MCMC step with gauge invariance check.
    /// </summary>
    public void BatchProposalStep(int proposalCount)
    {
        ...
        for (int p = 0; p < actualProposals; p++)
        {
            ...
            {
                double acceptProb = Math.Exp(-Beta * dS);
                accept = _randomNumbersCpu[p] < acceptProb;
            }
